...
async function getAdminsCount(adminId, filters, language) {
    try {
...
async function getAllAdminsInsideThePage(adminId, pageNumber, pageSize, filters, language) {
    try {
        const admin = await adminModel.findById(adminId);
        if (admin) {
            if (!admin.isBlocked) {
...
async function addNewAdmin(adminId, adminInfo, language) {
    try {
        const admin = await adminModel.findById(adminId);
        if (admin) {
...
async function updateAdminInfo(adminId, adminId, newAdminDetails, language) {
    try {
        const admin = await adminModel.findById(adminId);
        if (admin) {
            if (admin.isWebsiteOwner) {
...
async function deleteAdmin(adminId, adminId, language) {
    try {
        const admin = await adminModel.findById(adminId);
        if (admin) {
            if (admin.isWebsiteOwner) {
                if (!admin.isBlocked) {
                    if ((new mongoose.Types.ObjectId(adminId)).equals(adminId)) {
                        return {
                            msg: getSuitableTranslations("Sorry, Permission Denied Because This Admin Is Admin !!", language),
                            error: true,
                            data: {},
                        }
                    }
